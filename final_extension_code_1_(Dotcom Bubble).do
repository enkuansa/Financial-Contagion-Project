...
* Final Code: Extension (Forbes–Rigobon) PART 1 (DOTCOM BUBLE)
...
* 1. Further data cleaning using a loop
...
drop if missing(australia_ret_2day, canada_ret_2day, france_ret_2day, germany_ret_2day, hongkong_ret_2day, japan_ret_2day, netherlands_ret_2day, switzerland_ret_2day, uk_ret_2day, us_ret_2day, usNASDAQ_ret_2day, usa_3mtb, uk_3mtb, australia_rates, canada_3mtb, hongkong_rate, germany_3mtb, swizterland_3mtb) // drop any missing values that may be in the data still except for rates for Japan, Netherlands, and France
...
local stable_start  = td(01jan1995)
local stable_end    = td(09mar2000)
local turmoil_start = td(10mar2000) // NASDAQ Peak around that time.
local turmoil_end   = td(31dec2002) // NASDAQ trough point (October 9 2002) 
// but volatility persisted so I continued the data till the end of 2002
...
local p = 5 // number of lags for the returns variable  
local q = 5 // number of lags for the interest rates variable  
...
local cnames "Australia Canada France Germany HongKong Japan Switzerland UK"

local rets   "australia_ret_2day canada_ret_2day france_ret_2day germany_ret_2day hongkong_ret_2day japan_ret_2day switzerland_ret_2day uk_ret_2day"

local rates  "australia_rates canada_3mtb france_3mtb germany_3mtb hongkong_rate japan_rate swizterland_3mtb uk_3mtb"
...
forvalues i = 1/`K' {
    ...
    * 4.1 Estimate VAR only ONCE on FULL window and compute residuals
    preserve
        ...
        qui var usNASDAQ_ret_2day `rj', lags(1/`p') exog(L(1/`q').usa_3mtb L(1/`q').`ij')  // run the var model as described by Forbes and Rigobon.

        predict double eUS, resid equation(usNASDAQ_ret_2day) // predict the residuals for the US
